...
class LocationStore {
  ...
  @action async watchLocations(): Promise<boolean> {
    ...
    // Doing it for all locations, so files moved to another Location on disk, it's properly re-assigned in MetaTagger too
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (foundCreatedMatches.length > 0) {
        console.debug(
          `Found ${foundCreatedMatches.length} renamed/moved files in location ${location.name}. These are detected as new files, but will instead replace their original entry in the DB of MetaTagger`,
          foundCreatedMatches,
        );
